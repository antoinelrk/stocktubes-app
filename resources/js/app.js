...
const redrawTable = (filteredData) => {
    ...
    filteredData.forEach((tube) => {
        ...
        const type = tube.type === 'smc' ? 'smc' : 'tubes'

        incrementedLine += `
        <tr class="line" id="line">
            <td class="reference warning"><a href="/${type}/show/${tube.slug}">${tube.reference}</a></td>
            ...
            <td class="action">
                ...
                <a class=" ${!tube.datasheet ? "lock" : ""} target="_blank" href="/storage/datasheets/${type}/${tube.datasheet}">
                    <figure>
                ...
                <a class="btn-remove-danger js-delete-tube"
                   ${attrData}
                   data-message="Souhaitez-vous vraiment supprimer le ${type === 'smc' ? 'semi-conducteur' : 'tube'} ${tube.reference} ?"
                   data-payload="/${type}/delete/${tube.slug}"
                >
                    <figure>
